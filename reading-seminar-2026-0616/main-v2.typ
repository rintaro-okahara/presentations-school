...
#block(width: 100%, breakable: false, {
  ...
  grid(
    ...
    [5:], [#h(1.2em) Update $G_t = G_(t-1) + nabla_t nabla_t^top$ and define
      $ "(diagonal)" quad & H_t = arg min_(H succ.eq 0, space H = "diag"(H)) {G_t dot H^dagger + "Tr"(H)} = ("diag"(G_t))^(1/2) \
        "(full matrix)" quad & H_t = arg min_(H succ.eq 0) {G_t dot H^dagger + "Tr"(H)} = G_t^(1/2) $],
    [6:], [#h(1.2em) Update $y_(t+1) = x_t - eta H_t^dagger nabla_t$, #h(0.8em)
...
#text(size: 0.56em)[
  *Corrected form:* the source prints $"diag"(G_t^(1/2))$; the diagonal
  optimization gives $("diag"(G_t))^(1/2)$.
]
]

== Technical Convention: Singular AdaGrad Metrics

#[
  #set text(size: 0.74em)

  AdaGrad starts with $G_0 = 0$ and uses
  $H_t = G_t^(1/2)$ or $H_t = ("diag"(G_t))^(1/2)$, so $H_t$ can be singular.
  Throughout this section,
  $
    H^(-1) " denotes the Moore--Penrose pseudoinverse " H^dagger,
    quad
    norm(x)_H^2 = x^top H x
  $
  is generally a seminorm.

  #v(0.35em)

  The dual quantity uses a support-compatible extended-value convention:
  $
    norm(g)_(H,*)^2 :=
      g^top H^dagger g " if " g in "Range"(H),
      quad
      +infinity " otherwise."
  $
  Equivalently, when optimizing $A dot H^dagger$, impose
  $"Range"(A) subset.eq "Range"(H)$.

  #v(0.35em)

  This convention excludes degenerate choices such as $H = 0$ whenever the
  observed gradients are nonzero. Algorithm 19 is well-defined because
  $nabla_t in "Range"(H_t)$: in the full case
  $"Range"(H_t) = "span"{nabla_1, ..., nabla_t}$; in the diagonal case,
  $nabla_(t,i) != 0$ implies $(H_t)_(i i) > 0$.
]
...
#[
  ...
  $ H_t = arg min_H { G_t dot H^dagger + "Tr"(H) },
    quad G_t = sum_(s=1)^t nabla_s nabla_s^top . $

  The trace-normalized comparator class is obtained by separating the scale and
  shape of the metric.
  ...
  #text(size: 0.76em)[
    The dual seminorm is interpreted using the support-compatible pseudoinverse
    convention from the previous slide; otherwise the value is $+infinity$.
  ]
]
...
#[
  ...
  *Reading.* AdaGrad is nearly as good as the best fixed quadratic regularizer
  ...
  #text(size: 0.82em)[
    *Source typo.* The theorem statement in the source prints $eta = D_*$,
    while its proof--and direct minimization of
    $eta + D_*^2 / (2 eta)$--gives $eta = D_* / sqrt(2)$, with
    $D_* = D_oo$ or $D_* = D$.
  ]
...
#[
  ...
  #lemma("5.13")[
    $
      "Regret"_T
        <= eta / 2 (G_T dot H_T^dagger + "Tr"(H_T))
          + 1 / (2 eta) sum_(t=1)^T
  ...
  #text(size: 0.78em)[
    *Indexing correction:* the source prints $sum_(t=0)^T$, which would invoke
    undefined $x_0$ and $H_(-1)$; the proof uses $t = 1, ..., T$.
  ]
...
#[
  ...
  #grid(
    ...
    [
      ...
      Since $nabla_t in "Range"(H_t)$, the pseudoinverse acts as the inverse on
      the relevant subspace:
      $
        H_t H_t^dagger nabla_t = nabla_t.
      $

      #v(0.16em)

      From $y_(t+1)=x_t-eta H_t^dagger nabla_t$,
      $
        y_(t+1)-x^star
          &= x_t-x^star-eta H_t^dagger nabla_t, \
        H_t (y_(t+1)-x^star)
          &= H_t (x_t-x^star)-eta nabla_t.
      $
    ],
    [
      Multiplying the two displayed lines gives
      $
        norm(y_(t+1)-x^star)_(H_t)^2
          &= norm(x_t-x^star)_(H_t)^2
            -2 eta nabla_t^top (x_t-x^star) \
          &quad + eta^2 nabla_t^top H_t^dagger nabla_t. quad (5.8)
      $

      #v(0.16em)

      Since $x_(t+1)$ is an $H_t$-seminorm projection of $y_(t+1)$,
      $
        norm(x_(t+1)-x^star)_(H_t)^2
          <= norm(y_(t+1)-x^star)_(H_t)^2 .
      $
      Any minimizer gives this inequality, although the projection need not be
      unique.
    ],
  )
  ...
  $
    nabla_t^top (x_t-x^star)
      <= eta/2 nabla_t^top H_t^dagger nabla_t
      + 1/(2 eta)(
          norm(x_t-x^star)_(H_t)^2
          - norm(x_(t+1)-x^star)_(H_t)^2
        ). quad (*)
...
#[
  ...
  $
    sum_(t=1)^T nabla_t^top (x_t-x^star)
      <= eta/2 sum_(t=1)^T nabla_t^top H_t^dagger nabla_t
        + 1/(2 eta) S,
  $
  where
  $
  ...
  Hence, inserting the $t=1$ term with $H_0 = 0$, the usual telescope is
  interrupted exactly when the metric changes:
  $
    S
      &= - norm(x_(T+1)-x^star)_(H_T)^2 \
      &quad + sum_(t=1)^T (
...
#[
  ...
  Using $norm(z)_A^2 - norm(z)_B^2 = norm(z)_(A-B)^2$, the previous slide can
...
#[
  ...
  Define
  $
    Psi_t(H) = nabla_t nabla_t^top dot H^dagger,
    quad
    Psi_0(H) = "Tr"(H).
  $
  ...
  $
    sum_(t=1)^T nabla_t^top H_t^dagger nabla_t
...
#[
  ...
  $A succ.eq 0$, use the support-compatible PSD formulation:
  $
    (P_1) quad
    &min_(X succ.eq 0, "Tr"(X) <= 1, "Range"(A) subset.eq "Range"(X))
      A dot X^dagger, \
    (P_2) quad
    &min_(X succ.eq 0, "Range"(A) subset.eq "Range"(X))
      { A dot X^dagger + "Tr"(X) } .
  $
  ...
  *Exercise 11.* The trace-ball problem $(P_1)$ is exactly the exercise:
  ...
  For $A != 0$, Proposition 5.16 becomes
  $
    X_1^* = A^(1/2) / "Tr"(A^(1/2)),
    quad
    min(P_1) = "Tr"(A^(1/2))^2,
  ...
  If $A = 0$, both minima are $0$ and the case is trivial.
...
#[
  ...
  Restrict the argument to the support $"Range"(A)$. On this subspace the
  relevant matrices are positive definite, so ordinary inverses and Frobenius
  Cauchy--Schwarz apply. The orthogonal complement only increases $"Tr"(X)$.

  #v(0.25em)

  Let $S = "Tr"(A^(1/2))$. For feasible $X$ restricted to $"Range"(A)$,
...
#[
  ...
  Work on the support $"Range"(A)$ as before; an optimizer places no unnecessary
  mass on the orthogonal complement. Use the same inequality without the trace
  constraint. Put $t = "Tr"(X) > 0$ and $S = "Tr"(A^(1/2))$. Then
  $
    A dot X^(-1) >= S^2 / t.
  $
...
#[
  ...
  Thus the same support-compatible proof applies with $B = "diag"(A)$:
  $
    "trace-ball optimizer:" quad
      X_1^* = B^(1/2) / "Tr"(B^(1/2))
      = ("diag"(A))^(1/2) / "Tr"(("diag"(A))^(1/2)),
    \
  ...
  #text(size: 0.82em)[
    *Source typo.* The diagonal trace-ball denominator should be
    $"Tr"(("diag"(A))^(1/2))$, not $"Tr"(A^(1/2))$.
  ]
...
#[
  ...
  $
    G_T dot H_T^dagger
      = "diag"(G_T) dot ("diag"(G_T))^(dagger / 2)
      = "Tr"(("diag"(G_T))^(1/2))
      = "Tr"(H_T).
  $
  ...
  $ G_T dot H_T^dagger = "Tr"(H_T)$, so Lemma 5.14 follows immediately.
]
...
#[
  ...
  For diagonal AdaGrad,
  $"diag"(G_t) >= "diag"(G_(t-1))$ coordinatewise, so
  $("diag"(G_t))^(1/2) succ.eq ("diag"(G_(t-1)))^(1/2)$ and
...
#[
  ...
  In the full-matrix case, $G_t succ.eq G_(t-1)$. The matrix square-root map is
  operator monotone on the PSD cone:
  $A succ.eq B succ.eq 0 => A^(1/2) succ.eq B^(1/2)$.
  Hence $H_t - H_(t-1) succ.eq 0$, and
...
== Corrections and Technical Caveats in the Source

#[
  #set text(size: 0.54em)

  The main AdaGrad regret theorem remains valid, but Section 5.6 of the source
  contains several typographical or formulation issues.
  ...
  #grid(
    ...
    gutter: 0.8em,
    [
      - *Algorithm 19, diagonal metric.* Source: $"diag"(G_t^(1/2))$.
        Correct: $("diag"(G_t))^(1/2)$.
      - *Theorem 5.12, learning rate.* Source statement: $eta = D_*$.
        Correct for the displayed bound: $eta = D_* / sqrt(2)$. The source
        proof already uses the corrected value.
      - *Lemma 5.13, drift index.* Source: $sum_(t=0)^T$. Correct:
        $sum_(t=1)^T$.
    ],
    [
      - *Proposition 5.16, diagonal normalization.* Source denominator:
        $"Tr"(A^(1/2))$. Correct denominator:
        $"Tr"(("diag"(A))^(1/2))$.
      - *Proposition 5.16, singular matrices.* With an unrestricted
        Moore--Penrose inverse over $X succ.eq 0$, $X = 0$ makes the stated
        optimization false for nonzero $A$. Use a support condition, an
        extended-value convention, or a positive-definite approximation followed
        by a limit.
    ],
  )
  ...
  After these corrections, the proof yields exactly the same AdaGrad regret
  bound and the same $sqrt(d)$-factor improvement in the sparse-coordinate
  example.

  #v(0.2em)

  #text(size: 0.86em)[
    Source checked against Hazan, *Introduction to Online Convex Optimization*,
    2nd ed., arXiv:1909.05207v3, Sec. 5.6.
  ]
]

// Speaker notes:
// - Since the accumulated gradient matrix can be singular, the inverse here is
//   the Moore-Penrose pseudoinverse. The current gradient always lies in the
//   range of the AdaGrad metric, so the update acts like an ordinary inverse on
//   the relevant subspace.
// - There are a few minor typos and one support-related formulation issue in
//   the source. They do not affect the main theorem once the notation is made
//   precise.
// - Minimizing eta + D_*^2 / (2 eta) gives eta = D_* / sqrt(2), which is also
//   the value used in the source proof.
// - In the sparse-coordinate example, the AdaGrad upper bound is better by a
//   factor of sqrt(d), not by a factor of d.
